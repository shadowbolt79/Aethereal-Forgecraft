...
public class TileBloomery extends TileEntity implements ITickable{
    ...
    public int getNumCharcoal(){return inventory.getStackInSlot(0).stackSize+inventory.getStackInSlot(1).stackSize;}
    ...
    public void sendUpdate()
    {
        if(!worldObj.isRemote)
            Forgecraft.network.sendToAllAround(new PacketUpdateBloomery(this), new NetworkRegistry.TargetPoint(worldObj.provider.getDimension(), pos.getX(), pos.getY(), pos.getZ(), 64));
    }
    ...
    public boolean addMaterial(ItemStack material, boolean isCreative)
    {
        ...
        if(materialSmelting==null)
        {
            ...
            int iAdding = (material.stackSize*definition.getValue());

            int iRemainder = 0;
            while(iAdding>MAXMATERIAL)
            {
                iAdding = ((material.stackSize-(++iRemainder))*definition.getValue());
            }
            numSmelting=iAdding;
            if(!isCreative)
                material.stackSize=iRemainder;

            System.out.println("Bloomery: Added "+(iAdding)+ " "+materialSmelting.getName());
            if(temp>0)
                smeltingProgress=0;
            isOpen=false;
            markDirty();
            ...
            if(isOpen) {
                ...
                else
                {
                    ...
                    if(!thePlayer.inventory.addItemStackToInventory(stack)){
                        ...
                        thePlayer.worldObj.spawnEntityInWorld(new EntityItem(thePlayer.worldObj,thePlayer.posX,thePlayer.posY,thePlayer.posZ,stack));
                        thePlayer.setFire(5);
                    }
                }
    ...
    public void update() {
        ...
        if(isBurning())
        {
            ...
            System.out.println("Bloomery Temp: "+temp);
            ...
            if(timeSinceLastCharcoal > 120 || timeSinceLastCharcoal > (int)(1/(COALBURNA*Math.pow(COALBURNB,temp/100.0))))
            {
                ...
                if(getFilled()!=i)
                    updateFlag=true;
                System.out.println("Bloomery charcoal: "+getNumCharcoal());
                System.out.println("Bloomery time to next burn:"+(1/(COALBURNA*Math.pow(COALBURNB,temp/100.0))));
            }
